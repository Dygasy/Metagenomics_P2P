...
# -------------------------------
# Ensure DPP4 is a factor
meta$DPP4 <- factor(meta$DPP4, levels = c("no", "yes"))
...
# -------------------------------
# PERMANOVA
permanova <- adonis2(bray_dist ~ DPP4, data = meta)
print(permanova)
...
pcoa_df <- data.frame(pcoa$points, DPP4 = meta$DPP4)
colnames(pcoa_df)[1:2] <- c("PCoA1", "PCoA2")

plot_pcoa <- ggplot(pcoa_df, aes(x = PCoA1, y = PCoA2, color = DPP4)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(
    title = paste0("PCoA (Bray-Curtis) by DPP4\nPERMANOVA F=", permanova_F, ", p=", permanova_p),
...
pca_df <- data.frame(pca$x[, 1:2], DPP4 = meta$DPP4)
var_pca <- round(100 * (pca$sdev^2 / sum(pca$sdev^2))[1:2], 1)

plot_pca <- ggplot(pca_df, aes(x = PC1, y = PC2, color = DPP4)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(
    title = "PCA by DPP4",
...
nmds_df <- data.frame(nmds$points, DPP4 = meta$DPP4)

plot_nmds <- ggplot(nmds_df, aes(x = MDS1, y = MDS2, color = DPP4)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(title = "NMDS (Bray-Curtis) by DPP4")
...
plot(as.phylo(tree), type = "fan",
     tip.color = as.numeric(meta$DPP4),
     main = "UPGMA Tree (Bray-Curtis) by DPP4")

# -------------------------------
# ANOSIM
anosim_res <- anosim(bray_dist, grouping = meta$DPP4)
print(anosim_res)
...
ggplot(anosim_df, aes(x = Group, y = Distance, fill = Group)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = paste0("ANOSIM by DPP4\nR = ", anosim_R, ", p = ", anosim_p))
