...
# -------------------------------
# Ensure Insulin is a factor
meta$Insulin <- factor(meta$Insulin, levels = c("no", "yes"))
...
# -------------------------------
# PERMANOVA
permanova <- adonis2(bray_dist ~ Insulin, data = meta)
print(permanova)
...
pcoa_df <- data.frame(pcoa$points, Insulin = meta$Insulin)
colnames(pcoa_df)[1:2] <- c("PCoA1", "PCoA2")

plot_pcoa <- ggplot(pcoa_df, aes(x = PCoA1, y = PCoA2, color = Insulin)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(
    title = paste0("PCoA (Bray-Curtis) by Insulin\nPERMANOVA F=", permanova_F, ", p=", permanova_p),
...
pca_df <- data.frame(pca$x[, 1:2], Insulin = meta$Insulin)
var_pca <- round(100 * (pca$sdev^2 / sum(pca$sdev^2))[1:2], 1)

plot_pca <- ggplot(pca_df, aes(x = PC1, y = PC2, color = Insulin)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(
    title = "PCA by Insulin",
...
nmds_df <- data.frame(nmds$points, Insulin = meta$Insulin)

plot_nmds <- ggplot(nmds_df, aes(x = MDS1, y = MDS2, color = Insulin)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(title = "NMDS (Bray-Curtis) by Insulin")
...
plot(as.phylo(tree), type = "fan",
     tip.color = as.numeric(meta$Insulin),
     main = "UPGMA Tree (Bray-Curtis) by Insulin")

# -------------------------------
# ANOSIM
anosim_res <- anosim(bray_dist, grouping = meta$Insulin)
print(anosim_res)
...
ggplot(anosim_df, aes(x = Group, y = Distance, fill = Group)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = paste0("ANOSIM by Insulin\nR = ", anosim_R, ", p = ", anosim_p))
