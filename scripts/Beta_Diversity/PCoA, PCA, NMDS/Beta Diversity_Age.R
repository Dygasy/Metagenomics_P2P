...
# ---------------------------
# PERMANOVA by Ethnicity
permanova_eth <- adonis2(bray_dist ~ Ethnicity, data = meta)
print(permanova_eth)

# Extract F and p-values
permanova_F <- round(permanova_eth$F[1], 3)
permanova_p <- permanova_eth$`Pr(>F)`[1]

# ---------------------------
# PCoA Plot
pcoa <- cmdscale(bray_dist, eig = TRUE, k = 2)
var_explained <- round(100 * pcoa$eig / sum(pcoa$eig), 1)
pcoa_df <- data.frame(pcoa$points, Ethnicity = meta$Ethnicity)
colnames(pcoa_df)[1:2] <- c("PCoA1", "PCoA2")

plot_pcoa <- ggplot(pcoa_df, aes(x = PCoA1, y = PCoA2, color = Ethnicity)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(
...
pca_df <- data.frame(pca$x[, 1:2], Ethnicity = meta$Ethnicity)
var_pca <- round(100 * (pca$sdev^2 / sum(pca$sdev^2))[1:2], 1)

plot_pca <- ggplot(pca_df, aes(x = PC1, y = PC2, color = Ethnicity)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(
    title = "PCA by Ethnicity",
...
nmds_df <- data.frame(nmds$points, Ethnicity = meta$Ethnicity)

plot_nmds <- ggplot(nmds_df, aes(x = MDS1, y = MDS2, color = Ethnicity)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(title = "NMDS (Bray-Curtis) by Ethnicity")
...
plot(as.phylo(tree), type = "fan",
     tip.color = as.numeric(meta$Ethnicity),
     main = "UPGMA Tree (Bray-Curtis) by Ethnicity")

# ---------------------------
# ANOSIM
anosim_eth <- anosim(bray_dist, grouping = meta$Ethnicity)
print(anosim_eth)

# ANOSIM R and p-value
anosim_R <- round(anosim_eth$statistic, 3)
anosim_p <- anosim_eth$signif

# Construct distance group comparisons
anosim_dist <- as.matrix(bray_dist)
group_labels <- meta$Ethnicity
...
# ANOSIM boxplot
plot_anosim <- ggplot(anosim_df, aes(x = Group, y = Distance, fill = Group)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = paste0("ANOSIM by Ethnicity\nR = ", anosim_R, ", p = ", anosim_p))
