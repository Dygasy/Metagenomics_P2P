...
# Create Cholesterol category based on mmol/L
meta$Chol_cat <- cut(meta$Cholesterol, 
                     breaks = c(-Inf, 5.1, 6.1, Inf), 
                     labels = c("Desirable", "Borderline High", "High"))
meta$Chol_cat <- factor(meta$Chol_cat)

# Compute Bray-Curtis distance
bray_dist <- vegdist(otu_mat, method = "bray")

# PERMANOVA
permanova_chol <- adonis2(bray_dist ~ Chol_cat, data = meta)
print(permanova_chol)

# Extract F and p-values
permanova_F <- round(permanova_chol$F[1], 3)
permanova_p <- permanova_chol$`Pr(>F)`[1]

# PCoA
pcoa <- cmdscale(bray_dist, eig = TRUE, k = 2)
var_explained <- round(100 * pcoa$eig / sum(pcoa$eig), 1)
pcoa_df <- data.frame(pcoa$points, Chol_cat = meta$Chol_cat)
colnames(pcoa_df)[1:2] <- c("PCoA1", "PCoA2")

plot_pcoa <- ggplot(pcoa_df, aes(x = PCoA1, y = PCoA2, color = Chol_cat)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(
    title = paste0("PCoA (Bray-Curtis) by Cholesterol\nPERMANOVA F=", permanova_F, ", p=", permanova_p),
...
pca_df <- data.frame(pca$x[, 1:2], Chol_cat = meta$Chol_cat)
var_pca <- round(100 * (pca$sdev^2 / sum(pca$sdev^2))[1:2], 1)

plot_pca <- ggplot(pca_df, aes(x = PC1, y = PC2, color = Chol_cat)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(
    title = "PCA by Cholesterol Category",
...
nmds_df <- data.frame(nmds$points, Chol_cat = meta$Chol_cat)

plot_nmds <- ggplot(nmds_df, aes(x = MDS1, y = MDS2, color = Chol_cat)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(title = "NMDS (Bray-Curtis) by Cholesterol Category")
...
plot(as.phylo(tree), type = "fan",
     tip.color = as.numeric(meta$Chol_cat),
     main = "UPGMA Tree (Bray-Curtis) by Cholesterol Category")

# ANOSIM
anosim_chol <- anosim(bray_dist, grouping = meta$Chol_cat)
print(anosim_chol)

# ANOSIM R and p-value
anosim_R <- round(anosim_chol$statistic, 3)
anosim_p <- anosim_chol$signif

# Construct data frame for ggplot
anosim_dist <- as.matrix(bray_dist)
group_labels <- meta$Chol_cat
...
ggplot(anosim_df, aes(x = Group, y = Distance, fill = Group)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = paste0("ANOSIM by Cholesterol Category\nR = ", anosim_R, ", p = ", anosim_p))
