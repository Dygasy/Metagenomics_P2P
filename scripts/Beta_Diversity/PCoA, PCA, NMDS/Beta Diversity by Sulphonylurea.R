...
# -------------------------------
# Ensure Sulphonylurea is a factor
meta$Sulphonylurea <- factor(meta$Sulphonylurea, levels = c("no", "yes"))
...
# -------------------------------
# PERMANOVA
permanova <- adonis2(bray_dist ~ Sulphonylurea, data = meta)
print(permanova)
...
pcoa_df <- data.frame(pcoa$points, Sulphonylurea = meta$Sulphonylurea)
colnames(pcoa_df)[1:2] <- c("PCoA1", "PCoA2")

plot_pcoa <- ggplot(pcoa_df, aes(x = PCoA1, y = PCoA2, color = Sulphonylurea)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(
    title = paste0("PCoA (Bray-Curtis) by Sulphonylurea\nPERMANOVA F=", permanova_F, ", p=", permanova_p),
...
pca_df <- data.frame(pca$x[, 1:2], Sulphonylurea = meta$Sulphonylurea)
var_pca <- round(100 * (pca$sdev^2 / sum(pca$sdev^2))[1:2], 1)

plot_pca <- ggplot(pca_df, aes(x = PC1, y = PC2, color = Sulphonylurea)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(
    title = "PCA by Sulphonylurea",
...
nmds_df <- data.frame(nmds$points, Sulphonylurea = meta$Sulphonylurea)

plot_nmds <- ggplot(nmds_df, aes(x = MDS1, y = MDS2, color = Sulphonylurea)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(title = "NMDS (Bray-Curtis) by Sulphonylurea")
...
plot(as.phylo(tree), type = "fan",
     tip.color = as.numeric(meta$Sulphonylurea),
     main = "UPGMA Tree (Bray-Curtis) by Sulphonylurea")

# -------------------------------
# ANOSIM
anosim_res <- anosim(bray_dist, grouping = meta$Sulphonylurea)
print(anosim_res)
...
group_labels <- meta$Sulphonylurea
...
ggplot(anosim_df, aes(x = Group, y = Distance, fill = Group)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = paste0("ANOSIM by Sulphonylurea\nR = ", anosim_R, ", p = ", anosim_p))
