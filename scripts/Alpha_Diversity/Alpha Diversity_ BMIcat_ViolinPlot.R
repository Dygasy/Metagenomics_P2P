...
# -------------------------------
# 8. Plotting function for diversity metrics (with violin + boxplot + jitter)
plot_metric <- function(data, metric, y_label) {
  ggplot(data, aes(x = BMIcat, y = .data[[metric]], fill = BMIcat)) +
    geom_violin(trim = FALSE, alpha = 0.4, color = NA) +  # Add violin layer
    geom_boxplot(width = 0.1, outlier.shape = NA, alpha = 0.7) +  # Add boxplot layer
    geom_jitter(width = 0.15, size = 1.2, alpha = 0.5) +  # Add jitter points
    theme_minimal() +
    labs(title = paste(y_label, "Diversity"), y = y_label, x = "BMI Category") +
    stat_compare_means(method = "kruskal.test", label.x = 1, label.y.npc = "top") +
